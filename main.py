...
kw_exclusions = {
         'tv series':['2019', 'netflix' ,'top' ,'best' ,'watch' ,'2018' ,'bbc' ,'new']
       , 'tv show':[]
       , 'film':['2018' ,'favourite', '2019', 'camera' ,'news' ,'netflix' ,'window' ,'times', 'cling' ,'home'
              , 'cinema', 'polaroid', 'instax', 'izle']
       , 'movie':['box', '123', 'news', 'house', 'movies', 'cinema', 'putlocker', 'hindi']}

kw_dic, kw_list = gtf.kw_prep(keywords, kw_exclusions)


# Connect to Google
pytrends = TrendReq(hl='en-UK', tz=-60,timeout=(25,25), retries=5, backoff_factor=2)
...
tableau_df.to_gbq(destination_table = 'TSL03.Google_Trends_Tableau', project_id='skyuk-uk-nowtv-analytics-prod', chunksize=None, reauth=False, if_exists='replace', auth_local_webserver=False, table_schema=None, location=None, progress_bar=False, credentials=None, verbose=None, private_key=None)
...
IOT_df.to_gbq(destination_table = 'TSL03.Google_Trends_IOT', project_id='skyuk-uk-nowtv-analytics-prod', chunksize=None, reauth=False, if_exists='replace', auth_local_webserver=False, table_schema=None, location=None, progress_bar=False, credentials=None, verbose=None, private_key=None)
...
Daily_df = gtf.iot_periods('half','daily',1,pytrends,kw_list,kw_dic,kw_exclusions)

Weekly_df = gtf.iot_periods('year','weekly',6,pytrends,kw_list,kw_dic,kw_exclusions)
...
tableau_df.to_gbq(destination_table = 'TSL03.Google_Trends_IOT_DD', project_id='skyuk-uk-nowtv-analytics-prod', chunksize=None, reauth=False, if_exists='replace', auth_local_webserver=False, table_schema=None, location=None, progress_bar=False, credentials=None, verbose=None, private_key=None)