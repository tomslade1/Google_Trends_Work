...
def date_points(period):
    ...
    if period == 'week':
        ...
        raise ValueError("Invalid input: '"+period+"', please try one of 'week','month','quarter','half','year'")
...
def refactor_data(kw_dic,excl_dic,t,start,df, Final_df, period=''):
    ...
    for key, val in kw_dic.items():
        ...
        for v in val:
            ...
            # TODO: you're doing the same thing over and over here, and in the next section
            #       pop this into a function, put all your strings into an array and loop through this
            #       but should your loop be inside our outside of the function??

            regex_list = ["\suk$","\scast$","\s20\d{2}$","\sthe$", "\sthe$","\s{2}"]
            ...
            # When we did our 'groupby' aggregation earlier, it actually puts our defined column as the DF index so we need to reset this
            dataframe = dataframe.reset_index()

            # We want a couple of columns to tell us what time period we're looking at
    ...
    for query in kw:
        ...
        for k,v in excl_dic.items():
            ...
            if query == k:
                ...
                # If the list of exclusions is > 12 the API call won't work as Google kicks off, so we need to split up
                if excl_len > 12:

                    # Split up our list into n lists, by splitting at each 12th exclusion
                    composite_excl_list = [v[x:x+11] for x in range(0, len(v),11)]

                    # The final list won't have a full list of exclusions, which could return dodgy results so we pad out with previously used ones
                    missing_vals = 11 - len(composite_excl_list[-1]) 


                    rand_smpl = random.sample(composite_excl_list[0], k= missing_vals)

                    composite_excl_list[-1] = composite_excl_list[-1] + rand_smpl
...
def agg_table_refactor(input_df,kw_dic, final_df):
    ...
    for kw in kw_dic.keys():
            ...
            grouped_df = kw_df[['query','key_word','date_from','date_to','value','period']].groupby(['query','key_word','date_from','date_to','period']).sum()
...
def iot_periods(period_length,iteration_length,num_days,pytrends,kw_list,kw_dic,kw_exclusions):
    ...
    while start <= stop:
        ...
        if iteration_length.title() == 'Daily':
            ...
            if start.weekday() == 6 and start + datetime.timedelta(days=6) <= stop :
                date_list.append(start.strftime("%Y-%m-%d"))
            else:
                pass
            start = start + datetime.timedelta(days=1)
            ...
            ValueError("Invalid input: '"+iteration_length+"', please try either 'Daily'/'Weekly'")
    ...
    for dt in tqdm.tqdm_notebook(date_list):
        ...
        Final_df = refactor_data(kw_dic,kw_exclusions,t,start,df, Final_df, period=iteration_length)
